/* ============================================
   COURSEFORGE — APPLICATION LOGIC (OAuth Edition)
   ============================================ */
...
class CourseForge {
    ...
    init() {
        ...
        // Restore client ID in settings
        if (this.clientId) {
            this.els.clientIdInput.value = this.clientId;
        }

        // Show current origin in settings instructions
    ...
    waitForGIS() {
        if (typeof google !== 'undefined' && google.accounts) {
            ...
            // GIS library hasn't loaded yet, retry
    ...
    signIn() {
        ...
        if (this.tokenClient) {
            ...
            this.showToast('Failed to initialize Google sign-in. Check your Client ID.', 'error');
        }
    }
    ...
    async handleTokenResponse(response) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Show avatar, hide default if no picture
                if (!user.picture) {
                    this.els.userAvatar.style.display = 'none';
                } else {
                    this.els.userAvatar.style.display = 'block';
                }
            }
        } catch (err) {
    ...
    handleTokenError(error) {
        ...
        if (error.type === 'popup_closed') {
            ...
            this.showToast('Sign-in failed. Please check your Client ID in Settings.', 'error');
        }
    }
    ...
    openSettings() {
        ...
        // Update origin URL in case it changed
        this.els.originUrl.textContent = window.location.origin;
    }
    ...
    // ---- YouTube API (OAuth Bearer Token) ----
    async searchVideos(queries, maxPerQuery = 8) {
        ...
        for (const query of queries) {
            ...
            if (!res.ok) {
                ...
                throw new Error(`YouTube API error: ${res.status} — ${errData?.error?.message || 'Unknown error'}`);
            }
